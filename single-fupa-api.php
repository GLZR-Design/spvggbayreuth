...
<div id="widget_6023e42917d6f">... lade FuPa Widget ...
...
!(function (i) {
    ...
    t: 100,
    f: function () {
        team_widget(693178, { act: "tabelle", hea: 0, nav: 0, div: "widget_6023e42917d6f", mod: "0" });
    },
});
</script>
</div>
...
function team_widget(c, b) {
    var a = parseSettings(b);
    url = decodeURIComponent(window.location.hostname);
    urlNew = fupa_widget_domain + "/widget.php?val=" + c + "&p=start&act=" + a.act + "&fupa_widget_header=" + a.hea + "&fupa_widget_navi=" + a.nav + "&fupa_widget_div=" + a.div + "&url=" + url;
    
    console.log(b.div, urlNew);
}

function team_widget_klick(a, c) {
    var b = parseSettings(c);
    url = decodeURIComponent(window.location.hostname);
    scriptLaden(fupa_widget_domain + "/widget.php?val=" + a + "&p=klick&act=" + b.act + "&url=" + url);
}
...
function liga_widget_klick(b, c) {
    var a = parseSettings(c);
    url = decodeURIComponent(window.location.hostname);
    scriptLaden(fupa_widget_domain + "/widget.php?val=" + b + "&p=liga_klick&act=" + a.act + "&url=" + url);
}
function liga_widget_klick_gruppe(c, d, a) {
    var b = parseSettings(d);
    url = decodeURIComponent(window.location.hostname);
    scriptLaden(fupa_widget_domain + "/widget.php?val=" + c + "&p=liga_klick&act=" + b.act + "&journee_id=" + a + "&url=" + url);
}
...
function kreis_widget(c, b) {
    var a = parseSettings(b);
    url = decodeURIComponent(window.location.hostname);
    scriptLaden(fupa_widget_domain + "/widget.php?val=" + c + "&p=kreis_start&act=" + a.act + "&fupa_widget_div=" + a.div + "&fupa_widget_news_count=" + a.nec + "&url=" + url);
}
