...
BFVWidget.HTML5 = {
		...
		appPath : '/widget/widgetresource/iframe' + ('https:' == document.location.protocol ? '/ssl' : '' ) + '?url=' + 'http://www.spvgg-bayreuth.de',
		/****************************************
		...
		zeigeVereinsVideo : function(clubId, targetDivId, options){
			...
			var iFrame = BFVWidget.HTML5.createIFrame(options.height , options.width);
			iFrame.scrolling = 'no';
			var url = 'widget/club/videos/club' + clubId;
		...
		zeigeLigaVideo : function(compoundId, targetDivId, options){
			...
			var iFrame = BFVWidget.HTML5.createIFrame(options.height , options.width);
			iFrame.scrolling = 'no';
			var url = 'widget/league/videos/compound' + compoundId;
			if (options.videoCount) {
				url += '/videoCount' + options.videoCount;
			}else{
				url += '/videoCount0'
