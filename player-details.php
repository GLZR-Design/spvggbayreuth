...
if($socialMedia) {
    ...
    foreach ($socialMedia as $key => $value) {
        if($value) {
            $terms[$key] = "<a href='http://".$value."'><i class='fab fa-" . $key . "'></i></a>";
        }
    }
    $data["Social Media"] = implode( ' ', $terms );
}

$since = get_field('seit');
if($since) {
    $data["Altstädter seit"] = $since;
}
...
$output = '<dl class="sp-player-details">';

foreach( $data as $label => $value ):

	switch ($label) {
		case ("Größe") : 
			$labelEscaped = "groesse";
			break;
		case ("Fuß") : 
			$labelEscaped = "fuss";
			break;
		case ("Altstädter seit") : 
			$labelEscaped = "seit";
			break;
		default :
			$labelEscaped = $label;
	...
	$elemetClass = "sp-player-details--" . str_replace(' ', '-', strtolower($labelEscaped)) ;
	$output .= '<dt class="sp-player-details__label '. $elemetClass .'">' . $label . '</dt><dd class="sp-player-details__value '. $elemetClass .'">' . $value . '</dd>';
